...
import {vOr} from '../lang';

const { Vector } = ol.layer;
const { EventType } = ol.render;
const { Style, Icon} = ol.style;
const { GeometryType } = ol.geom;
const { Feature } = ol;
...
class HaloAnimationLayer extends Vector{
    ...
    constructor(opt){
        ...
        //光环动画属性
        this.lineWidth = vOr(opt.lineWidth, opt.lineWidth, 5);
        this.maxRadius = vOr(opt.maxRadius, opt.maxRadius, 40);
        this.minRadius = vOr(opt.minRadius, opt.minRadius, 15);
        this.radiusIncrement = vOr(opt.radiusIncrement, opt.radiusIncrement, 0.5);
        this.color = vOr(opt.color, opt.color, 'rgba(131, 45, 72,1)');
        ...
        this.backgroundCircleRadius = vOr(opt.outCircleRadius, opt.outCircleRadius, 10);
        this.frontCircleRadius = vOr(opt.innerCircleRadius, opt.innerCircleRadius, 10);
        this.canvasWidth = this.backgroundCircleRadius * 2 + 10;
        this.canvasHeight = this.backgroundCircleRadius * 2 + 10;
        this.backgroundCircleColor = vOr(opt.outCircleColor, opt.outCircleColor, "rgb(209, 211, 214)");
        this.frontCircleColor = vOr(opt.innerCircleColor, opt.innerCircleColor, "#e8a915");
        this.backgroundLineWidth = vOr(opt.outLineWidth, opt.outLineWidth, 4);
        this.frontLineWidth = vOr(opt.innerLineWidth, opt.innerLineWidth, 4);
