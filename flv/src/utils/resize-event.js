...
/* 判断浏览器是否有requestAnimationFrame对象 */
...
/* 判断浏览器是否有cancelAnimationFrame对象 */
...
vIf(!attachEvent && !isServer, () => {
	const testElement = document.createElement('fakeelement');
	vIf(testElement.style.animationName !== undefined, () => {
		animation = true;
	})
	vIf(animation === false, () => {
		let prefix = '';
...
/**
 * 给元素添加resize事件
 * @param {DOM_PREFIXES} element 添加的dom元素
 * @param {function} fn 移除的事件名称
 */
export const addResizeListener = function (element, fn) {
	...
	if (attachEvent) {
		...
		if (!element.__resizeTrigger__) {
			...
			vIf(animationStartEvent, () => {
				resizeTrigger.addEventListener(animationStartEvent, function (event) {
					vIf(event.animationName === RESIZE_ANIMATION_NAME, () =>{
						resetTrigger(element);
					})
				});
			})
...
/**
 * 为元素移除事件
 * @param {DOM_PREFIXES} element 移除的dom元素
 * @param {function} fn 移除的事件名称
 */
